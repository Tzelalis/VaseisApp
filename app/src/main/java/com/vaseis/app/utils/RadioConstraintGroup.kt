...
class ConstraintRadioGroup : ConstraintLayout {
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        ...
        // XML layout file
        val attributes = context.obtainStyledAttributes(attrs, R.styleable.constraint_radio_button, R.attr.radioButtonStyle, 0)

        //For debugging purposes
        /*saveAttributeDataForStyleable(context, R.styleable.ConstraintRadioGroup,
                attrs, attributes, R.attr.radioButtonStyle, 0);*/
        val value = attributes.getResourceId(R.styleable.constraint_radio_button_checkedButtonId, NO_ID)
        if (value != NO_ID) {
    ...
    override fun autofill(value: AutofillValue) {
        ...
        if (child == null) {
            Log.w(VIEW_LOG_TAG, "ConstraintRadioGroup.autoFill(): no child with index $index")
            return
        }
        check(child.id)
    }
